...
//made the dom focus on the selected element so that the default space works to
//submit the buttons
...
function selectNext() {
	...
	if (selected == null) {
		...
		document.getElementById( buttonOrder[0].substring( 1 ) ).focus();
	} else {
		index = buttonOrder.indexOf(selected);
		index = (index + 1) % buttonOrder.length;
		selectItem(buttonOrder[index])
		document.getElementById( buttonOrder[index].substring( 1 ) ).focus();
	}
}
...
$(document).keypress(function(event) {
	...
	if( event.key == "w" ) selectUp();
	else if( event.key == "s" ) selectDown();
	else if( event.key == "a" ) selectPrevious();
	else if( event.key == "d" ) selectNext();
})
